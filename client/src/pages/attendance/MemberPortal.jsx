...
import { 
  Home, Calendar, QrCode, User, Menu, ChevronLeft, ChevronRight, 
  LogOut, LifeBuoy
} from 'lucide-react';
...
export default function AttendanceMemberPortal({ onBack, user: propUser, token: propToken }) {
  ...
  const handleScanSuccess = (scanData) => {
    ...
    // You can add logic here to record attendance via API
  };
        ...
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-black text-slate-400 mb-2">Support Center</h3>
          ...
          <div className="flex items-center gap-3">
            ...
            <AnimatePresence mode="wait">
              {!isSidebarCollapsed && (
                ...
                >
                  <p className="text-white font-bold text-sm truncate">{currentUser?.name || 'Member'}</p>
                  <p className="text-green-300 text-xs font-bold uppercase tracking-widest">ID: {currentUser?.id || 'Loading...'}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
