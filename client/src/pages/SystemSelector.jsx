...
import { Database, QrCode, User, Shield, ClipboardList, Calculator } from 'lucide-react';
...
export default function SystemSelector({ onModuleSelect }) {
  ...
  const [secretaryLoginData, setSecretaryLoginData] = useState({ email: '', password: '' });
  const [treasurerLoginData, setTreasurerLoginData] = useState({ email: '', password: '' });
  ...
  const systems = [
    {
      ...
      features: ['Contribution Tracking', 'Claims Processing', 'Payment Schedules', 'Ledger Management']
    },
    {
      id: 'attendance',
      ...
      color: 'bg-coop-green',
      hoverColor: 'hover:bg-coop-green',
      features: ['QR Code Scanning', 'Real-time Tracking', 'Event Management', 'Reports & Analytics']
    }
  ];
  ...
  const handleAdminLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Call the parent callback to navigate to the appropriate portal
        if (onModuleSelect) {
          onModuleSelect(selectedSystem, selectedRole || 'admin', data.user, data.token);
        }
  ...
  const handleBack = () => {
    if (step === 'MEMBER_LOGIN' || step === 'ADMIN_LOGIN' || step === 'SECRETARY_LOGIN' || step === 'TREASURER_LOGIN') {
      setStep('ROLE_SELECTION');
      setLoginError('');
      setLoginData({ email: '', password: '' });
      setAdminLoginData({ email: '', password: '' });
