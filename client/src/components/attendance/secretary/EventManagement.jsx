...
import { 
  Calendar, Plus, Edit3, Trash2, MapPin, Clock, Users,
...
export default function EventManagement({ user, events, onRefreshEvents }) {
  ...
  const handleEditEvent = (event) => {
    ...
    setNewEvent({
      ...
      time: event.eventTime || eventDate.toTimeString().slice(0, 5),
  ...
  const handleUpdateEvent = async (e) => {
    ...
    try {
      ...
      const response = await eventAPI.updateEvent(selectedEvent.eventId || selectedEvent.id, eventData);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-950 tracking-tight">Event Management</h1>
          <p className="text-slate-500 text-sm font-bold mt-1">Create and manage General Assembly events</p>
        </div>
        <Button 
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader className="bg-slate-50">
                <TableRow>
                  <TableHead className="font-black uppercase text-[10px] tracking-widest text-slate-600">Event Details</TableHead>
                  <TableHead className="font-black uppercase text-[10px] tracking-widest text-slate-600">Date & Time</TableHead>
                  <TableHead className="font-black uppercase text-[10px] tracking-widest text-slate-600">Location</TableHead>
                ...
                {filteredEvents.map((event) => (
                  <TableRow key={event.id} className="hover:bg-slate-50/50 transition-colors">
                    <TableCell className="py-4">
                      <div>
                        <p className="text-sm font-black text-slate-900">{event.eventName || event.name}</p>
                        <p className="text-xs text-slate-500 font-bold">{event.type || 'General Assembly'}</p>
                      </div>
                    </TableCell>
                    <TableCell className="py-4">
                      <div>
                    ...
                    <TableCell className="py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-slate-900">{event.location}</span>
                      </div>
                    </TableCell>
                    <TableCell className="py-4">
                      <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(event.status)}`}>
      ...
      <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} className="max-w-lg">
        <h2 className="text-2xl font-black text-slate-950 mb-6">Create New Event</h2>
