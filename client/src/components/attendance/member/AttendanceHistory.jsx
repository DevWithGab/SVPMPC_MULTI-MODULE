...
export default function AttendanceHistory({ user, attendanceLogs }) {
  ...
  const filteredLogs = myLogs.filter((log) => {
    const eventName = log.eventName || log.event || "";
    const timestamp = log.scanTime || log.timestamp || log.date || "";
    const matchesSearch = eventName
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const logDate = new Date(timestamp);
    const matchesMonth =
      selectedMonth === "all" || logDate.getMonth() === parseInt(selectedMonth);
  ...
  const uniqueEvents = [
    ...new Set(myLogs.map((log) => log.eventName || log.event || "")),
  ].length;
  const averagePerMonth =
    totalAttendance > 0 ? Math.round(totalAttendance / 12) : 0;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">
            Complete record of your event attendance
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className="border-slate-200 hover:border-coop-green hover:bg-green-50 text-slate-600 hover:text-coop-green font-bold text-xs px-4 py-2 rounded-xl"
          >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {Object.entries(monthlyStats)
              ...
              .map(([month, count]) => (
                <div
                  key={month}
                  className="text-center p-4 bg-slate-50 rounded-2xl hover:bg-green-50 transition-colors"
                >
                  <p className="text-2xl font-black text-slate-950 mb-1">
                    {count}
                  </p>
      ...
      <Card className="border-slate-200/60 shadow-lg shadow-slate-200/40 rounded-[2rem] overflow-hidden bg-white">
        ...
        {filteredLogs.length > 0 ? (
          <Table>
            <TableHeader>
              <TableRow className="border-slate-100 hover:bg-slate-50/50">
                <TableHead className="font-black text-slate-600 text-[10px] uppercase tracking-[0.2em] py-6">
                  Date & Time
                </TableHead>
                <TableHead className="font-black text-slate-600 text-[10px] uppercase tracking-[0.2em]">
                  Event
                </TableHead>
                <TableHead className="font-black text-slate-600 text-[10px] uppercase tracking-[0.2em]">
                  Status
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-black text-slate-950 text-sm">
                          {new Date(
                            log.scanTime || log.timestamp || log.date || "",
                          ).toLocaleDateString()}
                        </p>
          ...
          <div className="p-12 text-center">
            ...
            <h4 className="text-lg font-black text-slate-400 mb-2">
              No Records Found
            </h4>
