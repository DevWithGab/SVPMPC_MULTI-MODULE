...
export default function AttendanceDashboard({
  ...
  const myLogs = Array.isArray(attendanceLogs) ? attendanceLogs : [];
  ...
  const handlePrint = () => {
    ...
    if (printWindow && qrRef.current) {
      ...
      const displayId = (user?.memberId || user?.id || user?.userId || "000000")
        .toString()
        .padStart(6, "0");
      ...
      printWindow.document.write(`
        <html>
          ...
          <body>
            <div class="card">
              <h1>${user?.name || user?.memberName || user?.username || "Member"}</h1>
              <p>Member ID: ${displayId}</p>
              ${svgData}
            </div>
  ...
  const handleDownload = () => {
    ...
    // Try PNG first by rendering the SVG onto a canvas
    ...
    img.onload = () => {
      try {
        ...
        link.href = png;
        link.download = `${(user?.name || "member").replace(/\s+/g, "-")}-qr.png`;
        ...
        const link = document.createElement("a");
        link.href = url;
        link.download = `${(user?.name || "member").replace(/\s+/g, "-")}-qr.svg`;
        document.body.appendChild(link);
        link.click();
        link.remove();
    ...
    img.onerror = () => {
      // If image loading fails, download raw SVG
  ...
  const thisMonth = myLogs.filter((log) => {
    const logDate = new Date(log.scanTime || log.timestamp);
    return (
  ...
  const uniqueEvents = [
    ...new Set(myLogs.map((log) => log.eventName || log.event || "Unknown")),
  ].length;
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-slate-500 text-sm font-bold mb-4">
                ...
                <span className="text-coop-green font-black">
                  {(user?.memberId || user?.id || user?.userId || "Loading...")
                    .toString()
                    .padStart(6, "0")}
                </span>
              </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <Card className="md:col-span-1 border-slate-200/60 shadow-xl rounded-[2rem] overflow-hidden bg-white text-center">
          <CardHeader className="bg-green-50 border-b border-green-100 pb-8 pt-8">
            <CardTitle className="text-xl font-black text-coop-darkGreen">
              Your QR Pass
            </CardTitle>
            <p className="text-coop-green/70 text-xs font-medium">
              Present this code for attendance
            </p>
          </CardHeader>
          <CardContent className="p-8 flex flex-col items-center">
            <div
              className="bg-white p-4 rounded-2xl shadow-inner border border-slate-100 mb-6"
              ref={qrRef}
            >
              <QRCodeSVG
                value={(
            ...
            <h3 className="text-2xl font-black text-slate-900">
              {user?.name || user?.memberName || "Loading..."}
            </h3>
            <p className="text-slate-500 font-mono mt-1 mb-6">
              ID:{" "}
              {(user?.memberId || user?.id || user?.userId || "000000")
                .toString()
                .padStart(6, "0")}
            </p>
            <div className="flex gap-3 w-full">
              <Button
                onClick={handlePrint}
                className="flex-1 h-12 rounded-xl bg-coop-green hover:bg-coop-darkGreen text-white font-black tracking-widest shadow-lg shadow-green-700/20"
              >
              ...
              <Button
                ...
                variant="outline"
                className="w-40 h-12 rounded-xl border-slate-200 hover:border-coop-green text-slate-700 font-black"
              >
                ...
                <TableBody>
                  {myLogs.slice(0, 5).map((log, index) => {
                    const logDate = new Date(
                      log.scanTime || log.timestamp || log.createdAt,
                    );
                    return (
                      <TableRow
                        key={log.attendanceId || log.id || log._id || index}
                      ...
                      >
                        ...
                        <TableCell className="py-4">
                          <span className="px-3 py-1 bg-green-100 text-coop-green rounded-full text-xs font-bold">
                            {log.eventName || log.event || "Attendance"}
                          </span>
                        </TableCell>
                        <TableCell className="py-4">
                    ...
                    <TableRow>
                      ...
                      >
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-xs">
                            Your attendance will appear here after scanning QR
                            codes.
                          </p>
                        </div>
                      </TableCell>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div className="text-left">
            <span className="block text-sm font-black uppercase tracking-[0.2em] text-slate-950 leading-tight">
              QR Scanner
            </span>
